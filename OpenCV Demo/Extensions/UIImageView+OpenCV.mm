...
- (CropRect)detectEdges {
    ...
    cv::resize(original, original, cv::Size(targetSize.width, targetSize.height));
...
void find_squares(cv::Mat& image, std::vector<std::vector<cv::Point>>&squares) {
    ...
    GaussianBlur(image, blurred, cv::Size(11,11), 0);//change from median blur to gaussian for more accuracy of square detection
...
- (UIImage *)crop: (CropRect)cropRect andApplyBW:(BOOL)applyBW {
    ...
    cv::Mat undistorted = cv::Mat( cv::Size(maxWidth,maxHeight), CV_8UC4);
    cv::Mat original = self.image.CVMat;
    
    cv::warpPerspective(original, undistorted, cv::getPerspectiveTransform(src, dst), cv::Size(maxWidth, maxHeight));
